...
- fixed: delegation JWT claims are not bound to task context. Delegated claims such as `parent_task_id`, `requested_by_agent`, `delegated_by`, depth, and actor context are checked against the submitted A2A task.
  Binding semantics: `delegated_by` represents the upstream requesting agent and must match the delegated task `fromAgent` or `requestedByAgent`; `requested_by_agent` must match `task.requestedByAgent`; `parent_task_id` must match `task.parentTaskId`. `mediatedBy` represents the Gateway/orchestrator mediator and is not bound unless a future explicit mediator JWT claim is added.
