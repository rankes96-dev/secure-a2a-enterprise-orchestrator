...
| trusted/onboarded connector agents | agent onboarding registry / current implementation | likely in-memory or state store depending current code | Installed connector trust may disappear and require re-onboarding. | Postgres durable connector registry | highest | Installed connector trust should survive orchestrator restart. |
