...
Browser-session mutating requests use a signed/session-bound double-submit CSRF cookie. `POST /session` issues the `ogen_csrf` CSRF cookie and the browser frontend must echo it in the `x-ogen-csrf-token` header for session-backed POST routes. The CSRF cookie remains readable by the frontend by design, defaults to `SameSite=Lax`, follows `SESSION_COOKIE_SAMESITE` for cross-site deployments, and is a Secure cookie in production. For the documented Vercel-to-Railway setup, set `SESSION_COOKIE_SAMESITE=None`; browsers require `SameSite=None` cookies to be Secure, so the session cookie becomes Secure automatically and `SESSION_COOKIE_SECURE=true` may also be set explicitly. The backend must be served over HTTPS. `CSRF_COOKIE_SAMESITE` can override only the CSRF cookie when needed; otherwise the CSRF cookie follows the session cookie's SameSite mode. Set `CSRF_SIGNING_SECRET` in production; `CSRF_TOKEN_TTL_SECONDS` optionally controls token lifetime. API-key and internal-service-token calls can bypass CSRF only when their configured secret matches; Authorization bearer alone is not a CSRF bypass.
...
Persisted audit viewer: when `PLATFORM_STATE_STORE_DRIVER=postgres` is enabled, browser users with the `audit.read` Gateway capability can read tenant-scoped safe audit projections through `GET /audit/events`. The viewer uses cursor pagination with a snapshot ceiling so new audit writes do not shift an open result window, and derived outcome/severity filters are evaluated before pagination through bounded ordered scans. If a sparse derived filter reaches the bounded scan limit, the API returns `422 audit_events_filter_scan_limit_exceeded` with safe operator guidance to narrow the time range, event type, conversation ID, or page limit. The error includes only safe counts, a filter hash, and classification strategy metadata; it never returns stored `safe_metadata`, raw prompts, OAuth tokens, JWTs, Authorization headers, private keys, client secrets, or client assertions. In local memory mode, audit viewer data is process-local and resets with the orchestrator. Future scale work should add a Gateway-produced materialized outcome/severity index or equivalent tenant-scoped read model while preserving cursor snapshots, `audit.read`, and the no-secrets projection boundary.
