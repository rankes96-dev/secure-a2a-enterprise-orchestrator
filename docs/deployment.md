...
Browser-session mutating requests use a signed/session-bound double-submit CSRF cookie. `POST /session` issues the `ogen_csrf` CSRF cookie and the browser frontend must echo it in the `x-ogen-csrf-token` header for session-backed POST routes. The CSRF cookie remains readable by the frontend by design, defaults to `SameSite=Lax`, follows `SESSION_COOKIE_SAMESITE` for cross-site deployments, and is a Secure cookie in production. For the documented Vercel-to-Railway setup, `SESSION_COOKIE_SAMESITE=None` makes both the session cookie and CSRF cookie cross-site compatible; `CSRF_COOKIE_SAMESITE` can override only the CSRF cookie when needed. Set `CSRF_SIGNING_SECRET` in production; `CSRF_TOKEN_TTL_SECONDS` optionally controls token lifetime. API-key and internal-service-token calls can bypass CSRF only when their configured secret matches; Authorization bearer alone is not a CSRF bypass.
